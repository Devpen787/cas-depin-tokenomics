...
Why employ a simulation-based stress-testing framework rather than traditional financial valuation models? Traditional models (DCF, Comparables) assume market efficiency and linear growth, which fail to capture the catastrophic non-linear feedback loops inherent in tokenized physical networks. 

Because we have defined Onocoy as a \textbf{Cyber-Physical System} (see Section \ref{subsec:depin_cps}), relying on static financial models is insufficient. As established by BlockScience and Gauntlet, these systems require engineering-grade verification to identify "edge case" failures that simple average-case forecasting obscures. Simulation allows us to test the physical resilience of the network (hardware retention) against the financial volatility of the token, properly treating the system as a closed loop requiring control-theoretic analysis.
...
\subsection{Person A Interview Framing Import (Provisional, Source-Preserving)}
\label{subsec:personA_methodology_import}
\input{sections/import_personA_methodology_interviews_2026-02-22.tex}
