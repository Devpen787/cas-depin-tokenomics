...
First, demand is modeled as an exogenous process. Although some scenarios allow demand to vary over time, the model does not fully endogenize user behavior or price elasticity. Second, providers are represented as economically rational agents operating under simplified decision rules. Strategic behavior such as long-term speculation or coordinated action is excluded.

Third, the price formation mechanism is a reduced-form approximation. It combines buy and sell pressure, dilution effects, scarcity signals, and stochastic noise to generate directional price dynamics. This approach is sufficient for comparative stress testing but does not attempt to replicate actual market microstructure. Fourth, the temporal resolution of the model is limited to discrete time steps, obscuring intra-period dynamics such as short-term volatility spikes.
...
Evaluating the robustness of tokenomic mechanisms requires metrics that are both interpretable and aligned with the functional objectives of DePIN networks. Metric selection in this thesis follows three principles.

First, metrics must be comparative, enabling evaluation of relative performance across mechanisms and scenarios rather than absolute success or failure. Second, metrics must be mechanism-relevant, reflecting provider incentives, service delivery, or economic sustainability rather than speculative market outcomes. Third, metrics must be transparent, with clearly defined computation and interpretation.
